...
for table_name in ("dependencies", "dev-dependencies", "build-dependencies"):
    for name, value in manifest.get(table_name, {}).items():
        ...
        if name.startswith("dekopon-") and version != "=0.15.0":
            raise SystemExit(f"error: {table_name}.{name} must be exactly =0.15.0")
...
expected = {
    ("dekopon-provider-sdk", "0.15.0"): "wit/provider.wit",
    ("dekopon-provider-http", "0.15.0"): "wit/deps/http.wit",
}
resolved = {}
...
comparisons = [
    (root / "wit/deps/provider.wit", resolved[("dekopon-provider-sdk", "0.15.0")]),
    (root / "wit/deps/http.wit", resolved[("dekopon-provider-http", "0.15.0")]),
]
...
for package in dekopon-provider-http dekopon-provider-sdk; do
  grep -Eq "^${package} v0\\.15\\.0([[:space:]]|$)" "$tree" || {
    echo "error: shipped graph does not contain crates.io $package 0.15.0" >&2
    exit 1
  }
done
